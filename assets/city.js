...
Promise.all([
  ...
  fetch('assets/comune.geojson').then(r => r.json())
    ...
    <div class="legend-item">
        <span class="legend-icon">${getCityIcon("porta")}</span>
        <span class="legend-label">
        <span class="it${lingua === 'en' ? ' d-none' : ''}">Porta</span>
        <span class="en${lingua === 'it' ? ' d-none' : ''}">Gate</span>
        </span>
    </div>

    <div class="legend-item">
        <span class="legend-icon">${getCityIcon("piazza")}</span>
        <span class="legend-label">
        <span class="it${lingua === 'en' ? ' d-none' : ''}">Spazio Pubblico</span>
        <span class="en${lingua === 'it' ? ' d-none' : ''}">Public Space</span>
        </span>
    </div>
    <div class="legend-item">
        <span class="legend-icon">${getCityIcon("chiesa")}</span>
        <span class="legend-label">
        <span class="it${lingua === 'en' ? ' d-none' : ''}">Chiesa</span>
        <span class="en${lingua === 'it' ? ' d-none' : ''}">Church</span>
        </span>
    </div>
    <div class="legend-item">
        <span class="legend-icon">${getCityIcon("palazzo")}</span>
        <span class="legend-label">
        <span class="it${lingua === 'en' ? ' d-none' : ''}">Palazzo</span>
        <span class="en${lingua === 'it' ? ' d-none' : ''}">Palace</span>
        </span>
    </div>
    `;
    return div;
    ...
    var sestieriLayer = L.geoJSON(sestieri, {
        onEachFeature: function(feature, layer) {
            cityFeaturesById[String(feature.properties.id)] = layer;
   ...
   function selectCityFeature(layer, feature) {
        if (selectedLayer) {
            resetSestiere(selectedLayer)
            selectedLayer = null;
        }
   ...
   function displayCurrentPage(results, tab, currentPage) {
        ...
        itemsToShow.forEach((doc) => {
            ...
            if (headElement){
                ...
                readLink = document.createElement('a');
                readLink.href = "https://statutiascoli.it/statuti.html?id=" + doc[1][0];
   ...
   function setUrlParam(categoryID) {
    ...
    function preselectCityFromUrl() {
        const rid = getIdFromUrl();
        if (!rid) return;

        const layer = cityFeaturesById[String(rid)];
        if (!layer) return;
        const feature = layer.feature;
        selectCityFeature(layer, feature);
    }
})
